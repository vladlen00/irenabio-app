...
// ASCII-подмножество. Прежний /^[^\s@]+@[^\s@]+\.[^\s@]+$/ пропускал КИРИЛЛИЦУ, а Lava такой
// адрес отбивает (400 "does not match email format") - женщина доходила до экрана оплаты и
// упиралась в тупик. WayForPay при этом тот же адрес принимает, то есть платёжки расходились.
// Разрешены НАМЕРЕННО: плюс-адресация (anna+irena@), поддомены, дефисы в домене, точки в
// локальной части. Правил про подряд идущие точки СОЗНАТЕЛЬНО нет: лишний отказ на экране
// оплаты дороже пропущенного адреса. Зеркала - normalizeEmail в create-lava-invoice и
// create-checkout; расходиться им нельзя, иначе сервер снова окажется слабее клиента.
function emailValid(email) {
  return email.length >= 6 && email.length <= 254
...
const MINI_APPS = {
  ...
  glutes: { url: "https://vladlen00.github.io/glutes/", v: "4", path: "/glutes/" },
...
async function doReset() {
  ...
  btn.disabled = true; btn.textContent = "Проверяем...";
  {
    ...
    // Заказ уже потрачен на смену пароля: одноразовость, второго сброса этим номером не будет.
    // Совета "войдите старым паролем" тут нет намеренно: женщина пришла именно потому, что его
    // не помнит. Единственный рабочий выход - поддержка.
    if (data.reason === "order_used") {
      showResetError(null, "Этим номером пароль уже меняли, второй раз он не работает. Напишите нам, откроем доступ: " + supportTgHtml());
      btn.disabled = false; btn.textContent = "Сбросить пароль";
      return;
    }
    if (res.status === 429) {
