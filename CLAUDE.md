...
3. **Удалить временные edge-функции** после проверки РФ-звука: `geo-probe, ru-loader, storage-probe, storage-list,
   ...
   `audio-probe` — ОТКРЫТАЯ проба звука без логина (verify_jwt=false, token-гейт `probe_audio_7c1e`): генерит свежую
   presigned на Timeweb/MinIO для `sprints/test/day1.m4a` + `<audio controls>`. URL:
   `https://kjzxrpwqyyjcykwbqskn.supabase.co/functions/v1/audio-probe?token=probe_audio_7c1e&host=timeweb`
   (`&host=minio` для A/B). Для РФ-тестеров (Алинчик/Ольга/Наталья) — открыть, нажать ▶. Удалить после подтверждения.
