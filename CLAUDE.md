...
- **ВСЕГДА бампать `?v=N`** у `app.js`/`style.css` в `index.html` (иначе кэш).
  Счётчики РАЗНЫЕ и бампаются независимо. Сейчас **app.js?v=67, style.css?v=47**.
...
## РЕДИЗАЙН: ГДЕ ОСТАНОВИЛИСЬ (прод app.js?v=67 / style.css?v=47)
...
**Хвост:** реальное значение `env(safe-area-inset-top)` проверить на живом iPhone с вырезом —
headless Chromium вырез не эмулирует, здесь подтверждено только то, что выражение валидно и
при нулевых insets даёт прежние 26px.
