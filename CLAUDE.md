...
- **ВСЕГДА бампать `?v=N`** у `app.js`/`style.css` в `index.html` (иначе кэш).
  Счётчики РАЗНЫЕ и бампаются независимо. Сейчас **app.js?v=69, style.css?v=49**.
...
Стоп затемнения сдвинут `62% → 78%`, `.home-subhead` сужен `290 → 270px` (Владлен,
2026-08-16: числа макета считались под другую, более тёмную слева картинку).
...
| обложка | было 62%/290 | стало 78%/270 |
|---|---|---|
| rejuvenation | 2.03 | **2.37** ← не берёт порог |
| glutes | 4.94 | 5.14 |
| sleep | 6.22 | 6.34 |
| antichaos | 6.28 | 6.34 |
| gut-body | 6.08 | 6.21 |
| nutrition | 4.67 | 5.16 |
| woman-body | 5.60 | 5.65 |

⚠️ **Лотос (rejuvenation) порог не берёт, и это ДЕЙСТВУЮЩИЙ спринт.** Причина: хвост
первой строки «и один» ложится на светлые лепестки. Перебором проверено, что
затемнением это не лечится — даже альфа `.28` со стопом 92% даёт лишь 3.55, а весь
цветок при этом тонет. Лечится ТОЛЬКО шириной: `max-width: 210px` даёт 4.56 и
проходит на всех семи, оставаясь в две строки (240 и 225 дают 3.92 — перелом строки
дискретен, значение меняется только когда слово целиком переезжает вниз).
Не менял сам: 270 — явное число Владлена.
...
## РЕДИЗАЙН: ГДЕ ОСТАНОВИЛИСЬ (прод app.js?v=69 / style.css?v=49)
